function gameStart() {
    ...
    //-----------------------------------------------BOMB------------------------------------------
    var bomb = {};
    $.extend(bomb, user(player.x, player.y, "#00A", 20, 30));
    bomb.active = false;
    bomb.draw = function() {
        canvas.fillStyle = this.color;
        canvas.fillRect(this.x, this.y, this.width, this.height);
    };
    bomb.inBounds = function() {
        return bomb.x >= 0 && bomb.x <= CANVAS_WIDTH &&
            bomb.y >= 0 && bomb.y <= CANVAS_HEIGHT;
    };
    ...
    bomb.update = function() {
        ...
        if (BOMB_INSTANTIATED) {
            bomb.x = player.x;
            bomb.y = player.y;
            BOMB_INSTANTIATED = false;
        }

        if (bomb.inBounds()) {
            bomb.x -= 8;
            console.log("is in bounds " + bomb.x);
            return bomb.active;
        } else {
            BOMB_INSTANTIATED = true;
            return false;
        }
    };
    ...
    function draw() {
        ...
        if (boss.active === false && bossKills > 300) {
            boss.active = true;
            setBossLocation();
            bossHP = 1000;
    ...
    function handleCollisions() {
        ...
        enemies.forEach(function(enemy) {
            if (collides(enemy, player)) {
                ...
                if (collides(bomb, enemy)) {
                    enemy.explode();
                    bossKills++;

                    enemies.forEach(function(enemy) {
                    var radius = 100;
                    if (bombCollides(enemy, bomb, radius)) {
                        enemy.explode();
                        bossKills++;
                    }
                  });
